...
import torch
import torch_gcu
from torch_gcu._utils import _take_tensors, _flatten_dense_tensors, \
...
def broadcast(tensor, devices=None, *, out=None):
    ...
    # TODO:When inter-card D2D is supported, remove the following line
    tensor = tensor.cpu()
    if devices is not None:
        devices = [_get_device_index(d) for d in devices]
        return torch_gcu._GCUC._broadcast(tensor, devices)
    else:
        return torch_gcu._GCUC._broadcast_out(tensor, out)


def broadcast_coalesced(tensors, devices, buffer_size=64 * 1024 * 1024):
    ...
    """
    devices = [_get_device_index(d) for d in devices]
    return torch_gcu._GCUC._broadcast_coalesced(tensors, devices, buffer_size)


def reduce_add(inputs, destination=0):
    ...
    for i, inp in enumerate(inputs):
        assert inp.device.type == "xla", "reduce_add expects all inputs to be on GCU,\
            but get device type:{}".format(inp.device.type)
    ...
    result = inputs[root_index] + nonroot[0].to(device=destination_device, non_blocking=False)
    for other in nonroot[1:]:
        result.add_(other.to(device=destination_device, non_blocking=False))
    return result

def reduce_add_coalesced(inputs, destination=0, buffer_size=64 * 1024 *1024):
    """Sums tensors from multiple GPUs.
    ...
    for tensor_at_dtus in zip(*inputs):
        if all(t.is_sparse for t in tensor_at_dtus):
            result = reduce_add(tensor_at_dtus, destination)  # this will be sparse too
            output.append(result)
            ref_order.append(tensor_at_dtus[0])
        else:
            for coll, t in zip(dense_tensors, tensor_at_dtus):
...
def scatter(tensor, devices=None, chunk_sizes=None, dim=0, *, out=None):
    ...
    if out is None:
        devices = [_get_device_index(d) for d in devices]
        return tuple(torch_gcu._GCUC._scatter(tensor, devices, chunk_sizes, dim))
    else:
        if devices is not None:
            raise RuntimeError(
        ...
        return tuple(torch_gcu._GCUC._scatter_out(tensor, out, dim))
...
def gather(tensors, dim=0, destination=None, *, out=None):
    ...
    if out is None:
        ...
        return torch_gcu._GCUC._gather(tensors_cpu, dim, destination)
    else:
        if destination is not None:
            raise RuntimeError(
                "'destination' must not be specified when 'out' is specified, but "
                "got destination={}".format(destination))
        return torch_gcu._GCUC._gather_out(tensors_cpu, out, dim)
