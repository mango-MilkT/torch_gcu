...
if dist.is_available():
    from torch_gcu.distributed.distributed_c10d import _get_default_group
    from torch_gcu.distributed.distributed_c10d import ReduceOp
from torch.nn.modules import Module
from .replicate import replicate
from .scatter_gather import scatter_kwargs, gather
from .parallel_apply import parallel_apply
from torch_gcu._utils import (
    _get_all_device_indices,
    _get_device_index
)
from torch_gcu.distributed.optim import ZeroRedundancyOptimizer
...
class DistributedDataParallel(Module):
    ...
    The same constraints on input as in :class:`torch_gcu.nn.DataParallel` apply.

    Creation of this class requires that ``torch.distributed`` to be already
    initialized, by calling :func:`torch.distributed.init_process_group`.

    ``DistributedDataParallel`` is proven to be significantly faster than
    :class:`torch_gcu.nn.DataParallel` for single-node multi-GPU data
    parallel training.
    ...
    Example::
        ...
        >>> net = torch_gcu.nn.DistributedDataParallel(model, pg)
    """
    def __init__(self, module, device_ids=None,
        ...
        if device_ids is not None and len(device_ids) > 1:
            raise RuntimeError("device_ids can only be None or contain a single element.")
        ...
        if self.device_type == "xla" and gradient_as_bucket_view :
            raise RuntimeError("dtu not support `gradient_as_bucket_view` set to True"
                               "please set gradient_as_bucket_view to False")
        self.dim = dim
        self.module = module
        ...
        self.broadcast_bucket_size = int(1024 * 1024 * 1024)

        # reduction bucket size
        self.bucket_bytes_cap = int(bucket_cap_mb * 1024 * 1024)

        # Sync params and buffers
        self._sync_params_and_buffers(authoritative_rank=-1)
    ...
    def _ddp_init_helper(self):
        ...
        bucket_indices = dist._compute_bucket_assignment_by_size(
            parameters[0],
            [self.bucket_bytes_cap, self.bucket_bytes_cap],
        ...
        # passing a handle to torch_gcu.nn.SyncBatchNorm layer
    ...
    def no_sync(self):
        ...
        Example::
            ...
            >>> ddp = torch_gcu.nn.DistributedDataParallel(model, pg)
            >>> with ddp.no_sync():
            >>>   for input in inputs:
    ...
    def forward(self, *inputs, **kwargs):
        ...
        if self.use_zero and not self.first_step:
            self.reducer.broadcast_zero_parameters()
        ...
        if self.use_zero and self.first_step:
            self.first_step = False
            world_size = dist.get_world_size(self.process_group)
            partition_parameters: List[List] = [[] for _ in range(world_size)]
            for rank in range(world_size):
                param_groups = self.zero_opt.partition_parameters()[rank]
                for param_group in param_groups:
                        for param in param_group["params"]:
                            partition_parameters[rank].append(param.data)
            self.reducer.set_zero_parameters(partition_parameters)
        return output

    def scatter(self, inputs, kwargs, device_ids):
        return scatter_kwargs(inputs, kwargs, device_ids, dim=self.dim)

    # singe process multi dtu need
    ...
    def join(self, divide_by_initial_world_size=True, enable=True):
        ...
        :class:`torch_gcu.nn.parallel.DistributedDataParallel` to be
        ...
        .. warning::
            ...
            of :class:`torch_gcu.nn.parallel.DistributedDataParallel`,
        ...
        Example::
          ...
          >>>  import torch_gcu.nn as nn
          >>>  # On each spawned worker
          >>>  def worker(rank):
          >>>      dist.init_process_group("nccl", rank=rank, world_size=2)
          >>>      torch.cuda.set_device(rank)
          >>>      model = nn.Linear(1, 1, bias=False).to(rank)
          >>>      model = torch_gcu.nn.parallel.DistributedDataParallel(
            ...
            if enable and not has_error:
                ...
                while not all_procs_joined:
                    ...
                    if num_active_procs == 0:
                        ...
                        # GCU not need rebuild bucket,because allreduce all gradients together
    ...
    def _register_comm_hook(self, state: object, hook: callable):
        ...
        """
        # TODO: need support ProcessGroupECCL::WorkECCL::getFuture
        if self.device_type == "xla":
            raise RuntimeError("dtu not support dist._register_comm_hook")
    ...
    def _distributed_broadcast_coalesced(
        ...
        for tensor in tensors:
            dist.broadcast(tensor, authoritative_rank, self.process_group)
    ...
    def _sync_params(self):
        with torch.no_grad():
            pass
            # these logics are moved to reducer.cpp for performance optimization
            # # module buffer sync
            # if self.will_sync_module_buffers():
    ...
    def set_zero_opt(self, zero: ZeroRedundancyOptimizer):
        self.zero_opt = zero
        self.use_zero = True
