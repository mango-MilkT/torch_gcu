...
class _MultiDeviceReplicator(object):
    ...
    def __init__(self, master_tensor: torch.Tensor) -> None:
        assert master_tensor.device.type == 'xla'
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def __init__(self,
        ...
        if self._enabled:
            ...
            self._use_zero_grad = use_zero_grad
    ...
    def _lazy_init_scale_growth_tracker(self, dev):
        ...
        self._scale = torch.full((1,), self._init_scale, dtype=torch.float32).to(dev)
        self._growth_tracker = torch.full((1,), self._init_growth_tracker, dtype=torch.int32).to(dev)
    ...
    def scale(self, outputs):
        ...
        if isinstance(outputs, torch.Tensor):
            assert outputs.device.type == 'xla'
    ...
    def unscale_(self, optimizer):
        ...
        found_inf = torch.full((1,), 0.0, dtype=torch.float32).to(self._scale.device)
    ...
    def _maybe_opt_step(self, optimizer, optimizer_state, *args, **kwargs):
        retval = None
        found_infs = list(optimizer_state["found_inf_per_device"].values())
        if self._use_zero_grad:
            found_inf = torch.stack(found_infs).sum()
            scaling_factor = (found_inf == 0).float()
            for grad in torch_gcu.fetch_gradients(optimizer):
                grad.nan_to_num_()
                grad.mul_(scaling_factor)
            retval = optimizer.step(*args, **kwargs)
        else:
            if len(found_infs) > 0 and found_infs[0].device.type == 'xla':
                torch_gcu.unlazy([], device=str(found_infs[0].device))
            if not sum(v.item() for v in found_infs):
                retval = optimizer.step(*args, **kwargs)
        return retval
    ...
    def update(self, new_scale=None):
        ...
        if new_scale is not None:
            ...
            if isinstance(new_scale, float):
                ...
                assert isinstance(new_scale.dtype, torch.FloatTensor), reason  # type: ignore[attr-defined]
    ...
    def _check_inf_per_device(self, optimizer):
        ...
        dummy_inv_scale = torch.full((1,), 1.0, dtype=torch.float32).to(_scale.device)
        found_inf = torch.full((1,), 0.0, dtype=torch.float32).to(_scale.device)
