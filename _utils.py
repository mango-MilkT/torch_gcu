...
def _rebuild_dtu_tensor(data, dtype, device, requires_grad):
...
def _reorder_tensors_as(tensors, ordered_tensors):
    ...
    for tensor in tensors:
        type_dict[tensor.dtype].append(tensor)
    type_dict = {t: iter(coll) for t, coll in type_dict.items()}
    return tuple(next(type_dict[tensor.dtype]) for tensor in ordered_tensors)
...
def _get_device_attr(get_member):
    ...
    if device_type.lower() == "xla":
        raise RuntimeError("GCU not support _get_device_attr")
...
def _get_current_device_index():
    ...
    dev = torch.device(torch_gcu._GCUC._dtu_get_default_device())
    return dev.index


def _get_all_device_indices():
    # all device index
    num = torch_gcu.device_count()
